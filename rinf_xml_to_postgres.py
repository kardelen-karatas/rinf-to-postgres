...
def process_sol(sol):
    ...
    SECTION_OF_LINE_OUTPUT_FILE.write(SOL_SQL_TEMPLATE % (SOL_length, SOL_nature, SOL_imcode, OPP_start , OPP_end, MEM_member, LIN_line ))

    EXI_start = sol.get("ValidityDateStart")
    EXI_end= sol.get("ValidityDateEnd")
    if EXI_start or EXI_end is not None:
        VALIDITY_DATE_OUTPUT_FILE.write(VALIDITY_DATE_SQL_TEMPLATE % (EXI_start,EXI_end)) 
...
def process_op(op):
    ...
    OPERATIONAL_POINT_OUTPUT_FILE.write(OP_SQL_TEMPLATE % (OPP_name, OPP_uniqueid, OPP_lon, OPP_lat, OPP_taftapcode, MEM_member))
...
def process_op_track(op_track):
    OTR_name = op_track.find('OPTrackIdentification').get("Value").replace("'", "''")
...
def process_op_tunnel(op_tunnel):
    OTU_name = op_tunnel.find('OPTrackTunnelIdentification').get("Value").replace("'", "''")
    OTU_imcode = op_tunnel.find('OPTrackTunnelIMCode').get("Value")
    OP_TUNNEL_OUTPUT_FILE.write(OP_TUNNEL_SQL_TEMPLATE % (OTU_name,OTU_imcode))
...
def process_op_platform(op_platform):
    OPL_name = op_platform.find('OPTrackPlatformIdentification').get("Value").replace("'", "''")
    OPL_imcode = op_platform.find('OPTrackPlatformIMCode').get("Value")
    OP_PLATFORM_OUTPUT_FILE.write(OP_PLATFORM_SQL_TEMPLATE % (OPL_name,OPL_imcode))
...
def process_sol_track(sol_track):
    STR_name = sol_track.find('SOLTrackIdentification').get("Value").replace("'", "''")
    STR_direction = sol_track.find('SOLTrackDirection').get("Value")
    SOL_TRACK_OUTPUT_FILE.write(SOL_TRACK_SQL_TEMPLATE % (STR_name,STR_direction))
...
def process_sol_tunnel(sol_tunnel):
    STU_name = sol_tunnel.find('SOLTunnelIdentification').get("Value").replace("'", "''")
    STU_imcode = sol_tunnel.find('SOLTunnelIMCode').get("Value")
    STU_startlon = sol_tunnel.find('SOLTunnelStart').get("Longitude").replace(",",".")
    STU_startlat = sol_tunnel.find('SOLTunnelStart').get("Latitude").replace(",",".")
    STU_endlon = sol_tunnel.find('SOLTunnelEnd').get("Longitude").replace(",",".")
    STU_endlat = sol_tunnel.find('SOLTunnelEnd').get("Latitude").replace(",",".")
    SOL_TUNNEL_OUTPUT_FILE.write(SOL_TUNNEL_SQL_TEMPLATE % (STU_name,STU_startlon,STU_startlat,STU_endlon,STU_endlat,STU_imcode))
...
context = etree.iterparse('RINFDataformat.xml')
fast_iter(context)
