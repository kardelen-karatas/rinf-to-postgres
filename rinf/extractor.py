    ...
    def __extract_sol(self, sol):
        ...
        # COUNT SOL_TUNNEL
            ...
            for soltunnel in soltunnels:
                SOL_tunnel_nb += 1
                STU_startlon = soltunnel.find('SOLTunnelStart').get(
                    "Longitude").replace(",", ".")
                STU_startlat = soltunnel.find('SOLTunnelStart').get(
                    "Latitude").replace(",", ".")
                STU_endlon = soltunnel.find('SOLTunnelEnd').get(
                    "Longitude").replace(",", ".")
                STU_endlat = soltunnel.find('SOLTunnelEnd').get(
                    "Latitude").replace(",", ".")
                soltunnel_parameters = soltrack.findall('SOLTunnelParameter')
...
if __name__ == '__main__':
    ...
    extractor.parse_xml(flags.rinf)
    extractor.close()
